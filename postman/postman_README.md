...
| A3 - Message Mapping (JSON to XML) | a3-pedido.json | Transformação JSON → XML |
...
| Request | Payload | Descrição |
|---|---|---|
| B1 - Router (ALTO valor) | b1-alto.json | Content-Based Router — rota de alto valor |
| B1 - Router (MEDIO valor) | ../payloads/b1-medio.json | Content-Based Router — rota de médio valor |
| B1 - Router (BAIXO valor) | b1-baixo.json | Content-Based Router — rota de baixo valor |
...
| Request | Payload | Descrição |
|---|---|---|
| C1 - Exception Subprocess (ordem VALIDA) | c1-ordem-valida.json | Fluxo sem erro → 200 |
...
| **SAP Developer Hub** | E2, E3, E4 | Portal separado do Integration Suite, necessário para criar Developer Apps e gerar Consumer Keys/Secrets; exige Role Collections específicas atribuídas no BTP Cockpit (ver troubleshooting em 21-e2-verify-api-key.md) |
