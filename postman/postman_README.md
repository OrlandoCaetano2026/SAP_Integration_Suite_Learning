...
| B1 - Router (MEDIO valor) | ../payloads/b1-medio.json | Content-Based Router — rota de médio valor |
| B1 - Router (BAIXO valor) | [b1-baixo.json](../payloads/b1-baixo.jsonta de baixo valor |
| B2 - Content Enricher | ../payloads/b2-pedido.json | Enriquecimento via lookup OData V4 |
| B3 - Splitter | ../payloads/b3-lote-itens.json | Divisão de lote em itens individuais |
| B4 - Aggregator | [b4-lote-8itens.json](../payloads/b4nsolidação de mensagens (CamelSplitComplete) |
| B5 - Multicast (Ordem de Producao) | [b5-ordem-producao.json](../payloads/b5-ordem-producao.jsona (MES/PLM/ERP) |
...
| C4 - Data Store (CREATE) | [c4-teste1-create.json](../payloads/Primeiro envio → 201 (registro criado) |
| C4 - Data Store (DUPLICATE) | [c4-teste2-duplicado.json.json](../payloads/c4-teste2-duplicado.json.idade detectada) |
| C4 - Data Store (UPDATE) | ../payloads/c4-teste3-update.json | Envio com `messageFunction` de atualização → 200 |
| C4B - Idempotent Process Call (CREATE/DUPLICATE) | reutiliza [c4-teste1-create.json](../payloads/c4-testeB (best practice SAP) — 1º envio 201, reenvio 409 via `CamelDuplicateMessage` |

#### Ⓓ Bloco D — Conectividade / Adapters

| Request | Payload | Descrição |
|---|---|---|
| D1 - OData (Germany + Sales Rep) | [d1-consulta-germany.ta-germany.json | Query OData dinâmica: filtra clientes alemães que são Sales Representative (**Northwind OData V4**, API pública) |
| D1 - OData (France) | ../payloads/d1-consulta-france.json | Filtro simples por país, ordenado por cidade |
| D1 - OData (nome contém Market) | [d1-consads/d1-consulta-nome.json | Busca por nome parcial usando `contains()` |
| D2 - SOAP Adapter (Nota Fiscal) | [d2-nota-fiscal.json](../l.json | Split/Gather com chamada a Web Service SOAP externo (**dataaccess.com NumberConversion**, API pública) |
...
| E2 - Verify API Key (D4 Vendor Validation via API Management) | reutiliza [d4-teste2-bloqueio-qualidade.json](../payloads/d4smo teste do D4, agora protegido pela Policy Verify API Key — requer preencher `{{consumer_api_key}}` com uma Consumer Key gerada no Developer Hub |
| E3 - Vendor Override (requer Bearer Token manual) | ../payloads/e3-vendor-override.json | Remove o bloqueio do fornecedor via `E3_VendorOverride_Proxy` — requer preencher `{{access_token_manual}}` com um Access Token OAuth (scope `vendor.write`), gerado previamente via `E3_OAuth_Token_Server_Proxy` |

⚠️ **Sobre E4 (Quota) e E5 (Spike Arrest):** esses dois cenários **não foram incluídos como requests prontos** na collection, propositalmente. Eles reutilizam o mesmo tipo de Access Token OAuth do E3, que expira automaticamente após ~30 minutos (`expires_in: 1799`) — um request salvo com um token fixo ficaria inutilizado rapidamente, exigindo sempre uma etapa manual de regeneração antes do teste.

Resumo de como esses cenários funcionam e foram executados na prática:
- **E3 (OAuth):** o consumidor faz um POST ao Proxy `E3_OAuth_Token_Server_Proxy` (`?grant_type=client_credentials`, Basic Auth com Consumer Key/Secret) e recebe um `access_token` (Bearer Token) válido por ~30 minutos. Esse token é usado no header `Authorization: Bearer <token>` para chamar o `D4_VendorValidation_Proxy` e o `E3_VendorOverride_Proxy` — cada um exigindo um Scope diferente (`vendor.read` ou `vendor.write`) conforme o Developer App que gerou o token.
...
O funcionamento completo, passo a passo, com todos os XMLs de Policy e evidências de teste, está documentado em ../docs/22-e3-oauth-scopes.md e [docs/23-e4-e5-quota-spike-arrest.md](../docs/23-e4-e5-quota-spike-arrest.mdernos
...
| **Neon** | D4, E1-E5 | Banco de dados PostgreSQL serverless gratuito; a tabela `vendor_block_status` precisa existir e estar populada conforme documentado em [19-d4-processprocessdirect.md |
| **SAP Developer Hub** | E2, E3, E4 | Portal separado do Integration Suite, necessário para criar Developer Apps e gerar Consumer Keys/Secrets; exige Role Collections específicas atribuídas no BTP Cockpit (ver troubleshooting em [21-e2-verify-api-key.md](../docsltar para o ../README.md
